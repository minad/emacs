...
load_seccomp (const char *file)
{
  ...
  size_t count = (size_t) stat.st_size / sizeof *program.filter;
  ...
  /* Try reading one more element to detect file size changes.  */
  program.filter = calloc (count + 1, sizeof *program.filter);
  if (program.filter == NULL)
    {
      emacs_perror ("calloc");
      goto out;
    }
  size_t read = fread (program.filter, sizeof *program.filter,
                       count + 1, stream);
  if (read != count || ferror (stream) != 0 || feof (stream) == 0)
    {
      fprintf (stderr,
  ...
  if (fclose (stream) != 0)
    perror ("fclose");  /* not a fatal error */
  stream = NULL;
  program.len = count;
